...
export function BatchFlow({ batch }: { batch: Batch }) {
  return <section className="batch-flow ops-section"><div className="panel-heading"><h2>{batch.name}</h2><span className="batch-state">{batch.status}{batch.mode === 'demo' ? ' · 演練' : ''}</span></div>
    ...
    <div className="batch-next"><ArrowRight size={18} /><p>{batch.status === '待接通' ? 'GM 工作佇列已建立；執行服務尚未接通，不會假稱已找貨或完成素材。' : batch.status === '待董事長核決' ? '選品已獲 GM 批准，可產製 Excel；社群等待董事長放行。' : batch.status === '已放行' ? batch.socialStatus === 'BLOCK' ? '社群已授權發布；外部服務未接通，發布 BLOCK。' : '選品已由 GM 批准；Excel 可交付，社群仍待內部流程。' : `目前：${gateDefinitions[Math.min(batch.gate, 7)].name}`}</p></div>
  ...
  <BatchDelivery batch={batch} /></section>;
}
