...
import { CandidateFollowup, FollowupSubmission } from './Followup';
...
export function SourcingResults({ batch }: { batch: Batch }) {
  ...
  return <section className="sourcing-results" aria-label={`${batch.name}選品結果`}>
    ...
    {!run ? <p>舊批次尚未執行供應資料選取，可立即接續。</p> : <>
      ...
      {!!run.candidates.length && <><p>來源候選已建立，採購與財務待核實。來源日期與本次執行時間分開記錄。</p><button className="quiet-button" onClick={() => download(run, `${batch.id}-candidates.json`)}>下載候選與證據</button><FollowupSubmission batch={batch} /><div className="sourcing-candidates">{run.candidates.map(r => <article className="sourcing-candidate" key={r.id}>
        ...
        <CandidateFollowup key={`${batch.id}-${r.id}`} batch={batch} record={r} />
      </article>)}</div></>}
    </>}
