...
export default function Dashboard() {
  ...
  const completed = state.batches.filter(b => b.status === '已放行');
  return <div className="brand-home">
    ...
    <div className="ops-counters">{[
      { name: '待社群核決', count: chairmanPending.length, to: '#current-batch', icon: ClipboardCheck },
    ...
    <div id="current-batch" className="home-active-batch">{active.map(batch => <div key={batch.id}><BatchDecision batch={batch} actor="董事長" /><BatchFlow batch={batch} /></div>)}</div>
    ...
    {completed.length > 0 && <details className="batch-details"><summary>已核決批次 · {completed.length}</summary><p className="subtle">核決不代表上架或發布完成；交付狀態請展開批次查看。</p>{completed.slice().reverse().map(batch => <BatchFlow key={batch.id} batch={batch} />)}</details>}
  </div>;
}
