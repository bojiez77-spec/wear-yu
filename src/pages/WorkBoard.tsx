...
export default function WorkBoard({ team }: { team: Team }) {
  ...
  return <>
    ...
    <p className="work-auto-note">由董事長發起批次，GM 依制度派工；本頁只查看執行進度。</p>
    ...
    {cases.length ? <div className="case-grid">{cases.map(item => <CaseCard key={item.id} item={item} />)}</div> : <div className="ops-empty">{filter === '全部' ? '尚未派工。請由董事長工作台發起新批次。' : `目前沒有${filter}的作業。`}</div>}
  </>;
}
