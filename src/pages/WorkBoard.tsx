...
export default function WorkBoard({ team }: { team: Team }) {
  ...
  return <>
    ...
    {team === '選品組' && <section className="approved-followups ops-section"><h2>已核准商品・獨立追蹤</h2><p className="subtle">依公司制度保留，不回候選池，也不等待新批次。</p>{["FREAK'S STORE for BEACH & FES Recycled Nylon Short Sleeve Shirt", "FREAK'S STORE Short Length Hoodie Knit"].map(name => <div className="followup-row" key={name}><strong>{name}</strong><span>核准已記錄 · 後續證據 OPEN</span><p>實際商品圖片、採購成本、規格庫存與上架證據待驗證；財務定價 BLOCK。官方 SKU 與淘寶登入不是阻擋原因。精確找款最多兩工作週期，之後轉同方向可採購替代貨源，保留原案例外。</p></div>)}</section>}
    {team === '選品組' && <><SupplierLibrary />{state.batches.map(batch => <SourcingResults key={batch.id} batch={batch} />)}</>}
