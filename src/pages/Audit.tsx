...
export default function AuditPage() {
  ...
  return <>
    ...
    {open && <form className="inline-form" onSubmit={e => {
      ...
      dispatch({ type: 'audit', id: caseId, finding: result === '有缺失' ? finding : '', eventId: crypto.randomUUID(), time: new Date().toLocaleString('zh-TW', { hour12: false }) });
    ...
    {state.audits.length ? <section className="ops-section">{state.audits.map(a => <article className="audit-row" key={a.id}><div><div className="case-meta"><span>{a.time}</span><Badge tone={a.resolved ? 'green' : 'gold'}>{!a.finding ? '無缺失' : a.resolved ? '已改善' : '待改善'}</Badge></div><h2>{a.title}</h2><p>{a.finding || '本次查核未發現缺失。'}</p></div>{a.finding && !a.resolved && <button className="gold-button" onClick={() => { dispatch({ type: 'resolve', id: a.id }); setNotice('已確認改善，原始缺失紀錄保留。'); }}>確認已改善</button>}</article>)}</section> : <div className="ops-empty">尚無查核紀錄。建立作業後即可進行查核。</div>}
  </>;
}
