...
- Current implementation stores local browser records only. It is not a shared, authenticated approval system. Shopee Excel delivery ends at download for manual upload; it is not an integration failure. Social provider adapters in the website are not configured: publication jobs stay BLOCK. Never infer successful upload or publication from local records or a user's editable state. Do not expose credentials in browser storage, imports, logs, or repository files.

## 14. Chairman scope update — 2026-09-08
