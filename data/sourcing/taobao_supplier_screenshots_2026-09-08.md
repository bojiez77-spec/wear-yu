# Wear-Yu Taobao priority supplier screenshots — 2026-09-08

Source: supplier/store screenshots supplied directly by Chairman during 2026-09-08 01:03–01:07 Asia/Taipei.
...
| File | Search/store context visible | Store / identifier | Priority notes |
|---|---|---|---|
| IMG_0099.jpeg | `dotair` product grid | dotair context | Priority supplier discovery material. |
| IMG_0100.jpeg | `dotair` store home | SHUFFLE STYLE STORE | Priority store; hot searches include `searoom`, `cgs`, `supplex`, `anchor衣服`, `狐狸卫衣`, `snpk`. |
| IMG_0101.jpeg | search `短袖` | 大树树 外单店 | Priority supplier. |
| IMG_0102.jpeg | product grid | 大树树 外单店 context | Priority supplier product-grid evidence. |
...
| IMG_0126.jpeg | search `freaks store` | 哒哒哒家 context | Priority supplier product-grid evidence; search phrase alone does not prove brand identity. |

The Chairman described the complete set as **28 screenshots**. The manifest above records the 18 screenshot files whose filenames are currently concretely available in this operating context. The remaining Chairman-provided screenshots remain part of the declared priority supplier pool, but Codex must not invent filenames/store mappings for any file not concretely identified. Add them when exact file/store evidence is available.

## Required Codex sourcing behavior
...
## Registered local source filenames
